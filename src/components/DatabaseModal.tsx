...
export const DatabaseModal: React.FC<DatabaseModalProps> = ({
    ...
    try {
      ...
      if (result) {
        ...
        if (loadResult.success) {
          onShowToast(
            `Base de datos "${result.file.name}" cargada (${loadResult.stats?.products || 0} productos)`,
            'success'
          );
          onClose();
        } else {
  ...
  const handleCreateWithSampleData = async () => {
    ...
    try {
      ...
      onShowToast('Base de datos de demostración cargada', 'success');
      onClose();
    } catch (err: any) {
  ...
  const handleDownloadCopy = () => {
    ...
    onShowToast(`Archivo "${fileName}" descargado`, 'success');
  };
  ...
  const handleBindWritableHandle = async () => {
    ...
    try {
      ...
      if (handle) {
        ...
        await dbService.saveCurrentDatabase();
        onShowToast(`Archivo "${handle.name}" vinculado`, 'success');
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSelectFolder = async () => {
    ...
    try {
      ...
      if (dirResult && dirResult.fileHandle) {
        ...
        await dbService.saveCurrentDatabase();
        onShowToast(`Carpeta vinculada`, 'success');
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleDisconnect = () => {
    if (
      window.confirm(
        '¿Deseas cerrar la base de datos actual? Tus datos seguirán guardados.'
      )
    ) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 z-50 overflow-y-auto">
        <div className="bg-white rounded-2xl max-w-xl w-full shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[94vh] my-auto">
          {/* Header */}
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Archivo independiente (.json) para pendrive o copia de seguridad
                </p>
              </div>
            </div>
            <button
          ...
          <div className="p-5 space-y-4 overflow-y-auto flex-1 text-xs">
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-extrabold text-slate-900 font-mono">
                      {isLoaded ? dbFileName : 'Ninguna base de datos'}
                    </span>
                  </div>
                </div>
                ...
                {isLoaded && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800">
                    {hasWritable ? 'Guardado directo' : 'Guardado interno'}
                  </span>
                )}
              </div>
              ...
              {isLoaded ? (
                <div className="mt-3 pt-3 border-t border-emerald-100/80 space-y-2">
                  ...
                  <div className="bg-white/90 p-2.5 rounded-lg border border-emerald-100 space-y-1 text-[11px]">
                    <div className="flex items-center justify-between text-slate-600">
                      ...
                      <span
                        className="font-mono text-emerald-800 font-bold max-w-[260px] truncate"
                        title={dbPath || dbFileName || ''}
                      >
                    ...
                    <div className="flex items-center justify-between text-slate-600 border-t border-slate-100 pt-1">
                      ...
                      <span className="font-mono text-emerald-800 font-bold max-w-[220px] truncate">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] text-slate-600 pt-1">
                    <span>
                      {lastSavedAt
                        ? `Último guardado: ${formatDateTime(lastSavedAt.toISOString())}`
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-2">
                    ...
                    >
                      ...
                      <span>{hasUnsaved ? 'Guardar ahora' : 'Todo guardado'}</span>
                    </button>

                    {/* NUEVO BOTÓN: Editar empresa */}
                    <button
                      type="button"
                      id="btn-modal-edit-company"
                      onClick={() => setIsCompanyModalOpen(true)}
                      className="px-3 py-1.5 bg-amber-100 hover:bg-amber-200 text-amber-900 rounded-lg font-bold flex items-center gap-1.5 shadow-xs transition cursor-pointer text-xs border border-amber-300"
                      title="Editar los datos que aparecen en la factura"
                    >
                ...
                <p className="text-xs text-slate-500 mt-2">
                  Para comenzar, selecciona un archivo existente o crea uno nuevo.
                </p>
              )}
            </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">
                      Elige un archivo .json de tu pendrive o disco duro
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs space-y-2.5">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">
                    Empieza desde cero con un archivo limpio
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder="Nombre (ej: inventario.json)"
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-xs font-bold text-amber-950">
                    Crear con Datos de Ejemplo
                  </h4>
                  <p className="text-[11px] text-amber-800">
                    Genera un archivo con paneles e inversores
                  </p>
                </div>
              </div>
            ...
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex items-start gap-2 text-[11px] text-slate-600">
              ...
              <p>
                <strong>100% Portátil:</strong> copia tu archivo a un pendrive, súbelo
                a Drive o envíalo por email. Al abrirlo en otro ordenador, verás todos
                tus datos.
              </p>
            </div>
          </div>
      ...
      {/* Modal de edición de empresa (anidado) */}
      <CompanyModal
